...
namespace jm {
    ...
    namespace detail {
        ...
        // loads up to 8 characters of string into std::uint64 and xors it with the key
    ...
    public:
        ...
        XORSTR_FORCEINLINE void crypt() noexcept
        {
            ...
            ((Indices >= sizeof(_storage) / 16 ? static_cast<void>(0) : __builtin_neon_vst1q_v(
                                    ...
                                    veorq_u64(__builtin_neon_vld1q_v(reinterpret_cast<const std::uint64_t*>(_storage) + Indices * 2, 51),
                                              __builtin_neon_vld1q_v(reinterpret_cast<const std::uint64_t*>(keys) + Indices * 2, 51)),
                                    51)), ...);
#else // GCC, MSVC
            ((Indices >= sizeof(_storage) / 16 ? static_cast<void>(0) : vst1q_u64(
                        reinterpret_cast<uint64_t*>(_storage) + Indices * 2,
                        veorq_u64(vld1q_u64(reinterpret_cast<const std::uint64_t*>(_storage) + Indices * 2),
                                  vld1q_u64(reinterpret_cast<const std::uint64_t*>(keys) + Indices * 2)))), ...);
#endif
